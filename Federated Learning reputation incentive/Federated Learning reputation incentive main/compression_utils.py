...
def dgc(T, hp):
  '''
  Thresholding Compression: Directly apply thresholding compression based on the provided threshold.
  '''
  # Fixed threshold value (no need for external parameter)
  threshold_value = 0.0001  # Set the fixed threshold to 0.5 (you can change this value if needed)

  # Apply thresholding compression
  T_abs = torch.abs(T)

  # Keep elements greater than or equal to threshold, set others to zero
  out = torch.where(T_abs >= threshold_value, T, torch.zeros_like(T))

  return out

# def dgc(T, hp):  # 随机选择
#   '''
#   Random Sampling Compression: Randomly sample a fixed fraction of elements.
#   '''
#   # Fixed sampling ratio (no need for external parameter)
#   sampling_ratio = 0.1  # Set the fixed sampling ratio to 10% (you can change this value if needed)
#
#   # Get the number of elements to sample
#   n_elements = T.numel()
#   n_sample = int(n_elements * sampling_ratio)
#
#   # Generate random indices to select elements
#   indices = torch.randint(0, n_elements, (n_sample,))  # Randomly select indices from the range [0, n_elements)
#
#   # Create a mask where only the sampled indices are kept as 1, others are 0
#   mask = torch.zeros_like(T).flatten()
#   mask[indices] = 1
#
#   # Apply the mask to keep only sampled elements, set others to zero
#   out = T * mask.view_as(T)
#
#   return out
